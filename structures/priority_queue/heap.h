...
namespace structures
{
	...
	void Heap<T>::push(int priority, const T& data)
	{
		...
		{
			Utils::swap(
				list_->at(indexCurrent),
				list_->at(indexParent)
			);
	...
	T Heap<T>::pop()
	{
		...
		if (index != -1)
		{
			...
			if (index != indexLast)
			{
				Utils::swap(PriorityQueueList<T>::list_->at(index),
			...
			while (indexSon != -1 && PriorityQueueList<T>::list_->at(indexCurrent)->getPriority() > PriorityQueueList<T>::list_->at(indexSon)->getPriority())
			{
				Utils::swap(PriorityQueueList<T>::list_->at(indexCurrent),
