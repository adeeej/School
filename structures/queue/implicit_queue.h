...
namespace structures
{
	...
	inline bool ImplicitQueue<T>::equals(Structure& other)
	{
		...
		if (size_ != otherQueue->size_)
		{
			return false;
		}

		size_t thisIndex = startIndex_;
		size_t otherIndex = otherQueue->startIndex_;

		for (size_t i = 0; i < size_; i++)
		{
			if (array_->at(thisIndex) != otherQueue->array_->at(otherIndex))
			{
				return false;
			}
	...
	inline void ImplicitQueue<T>::push(const T& data)
	{
		...
		if (size_ < capacity)
		{
			array_->at(static_cast<int>((startIndex_ + size_) % capacity)) = data;
			size_++;
		}
		else
		{
