...
namespace structures
{
	...
	inline void SortedSequenceTable<K, T>::insert(const K& key, const T& data)
	{
		...
		{
			throw std::logic_error("Table contains this key");
		}
	}
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(K key, int indexStart, int indexEnd, bool& found)
	{
		...
		K middleKey = SequenceTable<K, T>::list_->at(middleIndex)->getKey();
		...
		else
		{
			...
			{
				found = false;
				return key < middleKey ? middleIndex : middleIndex + 1;
			}
			else
			{
				if (key < middleKey)
				{
					//return indexOfKey(key, indexStart, middleIndex - 1, found);
