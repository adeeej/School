...
namespace structures
{
	...
	inline Structure& BinarySearchTree<K, T>::assign(Structure& other)
	{
		if (this != &other)
		{
			...
			Iterator<TableItem<K, T>*>* itB = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(otherBst.binaryTree_->getRoot());
			Iterator<TableItem<K, T>*>* itE = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(nullptr);
	...
	inline void BinarySearchTree<K, T>::insert(const K& key, const T& data)
	{
		...
		if (!tryToInsertNode(treeNode))
		{
			delete tableItem;
			delete treeNode;
			throw std::logic_error("Key is already present in the table!");
		}
	}
